...
ggplot(choices_combined, aes(x = label, y = percent, fill = choices)) +
  ...
  scale_y_continuous() +
  labs(x = "", y = "Proportion of ants", fill = "Choices") +  # legend title
  theme_minimal(base_size = 14) +
  theme(
    axis.text.x = element_text(size = 13),
    axis.title.y = element_text(size = 22),
    axis.text.y = element_text(size = 18),# bigger y-axis label
...
ggplot(abandon_combined, aes(x = label, y = percent, fill = abandonment)) +
  ...
  theme_minimal(base_size = 14) +
  theme(
    axis.text.x  = element_text(hjust = 0.5, size = 16), # straight x-axis labels, bigger size
    axis.text.y  = element_text(size = 16),              # bigger y-axis numbers
...
```{r}
#Low sucrose
gaster_angle_low_sucrose_wrong_choices_unfed<-subset(gaster_angle_low_sucrose_ready,Ant.fed.on.the.feeder=="No" & Ant.choice=="Wrong")

gaster_angle_low_sucrose_correct_choices<-subset(gaster_angle_low_sucrose_ready,Ant.choice=="Correct")

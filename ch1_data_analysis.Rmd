...
###################################################################
ANALYZING CHOICE and ABANDONMENT DATA FOR 1.5M EXPERIMENT
###########################################################
...
Whenever ant makes a choice (correct or wrong), it stays on the trail and when it abandons it does not. So transforming the data accordingly and combining the dataframes to analyze abandonment
...
#################################################################
ANALYZING CHOICE AND ABANDONMENT DATA FOR 3M EXPERIMENT
##################################################################
...
choices_plot<-ggplot(new_df)+
  ...
  geom_text(stat = "prop", position = position_fill(.5),size=7)+
...
ggplot(new_df,aes(x=exp_type,fill=choices))+
...
sample_size_1<-as.data.frame(all_exp_combined_abandonment %>%
  group_by(exp_type,treatment)%>%
  summarize(total=n()))


#Plotting both experiments abandonment data together
ggplot(all_exp_combined_abandonment,aes(exp_type))+
geom_bar(aes(fill=abandonment))+
  xlab("Treatment")+
  ylab("Total number of ants")+
  theme(text=element_text(size=20),axis.text = element_text(size=12))+scale_x_discrete(labels=c("Control_1.5m"="Control 1.5M","Control_3m"= "Control 3M", "Exp_1.5m"="Experimental 1.5M","Exp_3m" = "Experimental 3M"))+geom_text(data=sample_size,aes (x=exp_type,y=60),label=paste("n=",sample_size_1$total))

#Another way to plot
library(ggstats)

new_df_1<-as.data.frame(all_exp_combined_abandonment %>%
  group_by(exp_type,abandonment)%>%
  summarize(total=n()))

choices_plot_1<-ggplot(new_df_1)+
  aes(exp_type,fill=abandonment,weight=total,by=exp_type)+
  geom_bar(position="fill")+
  geom_text(stat = "prop", position = position_fill(.5),size=7)+
  scale_y_continuous(labels = scales::percent)+scale_x_discrete(labels=c("Control_1.5m"="Control 1.5M","Control_3m"= "Control 3M", "Exp_1.5m"="Experimental 1.5M","Exp_3m" = "Experimental 3M"))+
  xlab("Treatments")+
...
choices_plot_1+theme(text=element_text(size=25),axis.text = element_text(size=17))
...
chisq.test(all_exp_combined_control_abandonment,correct=F)
#Non-significant
...
chisq.test(all_exp_combined_exp_abandonment,correct=F)
...
############################################################################ ANALYZING GASTER ANGLES BETWEEN CONTROL AND EXPERIMENTAL TREATMENTS FOR 1.5M AND 3M DATA ###################################
...
combined_gaster_angle$vertical_mean_angle<-90-(combined_gaster_angle$mean_gaster_angle)

wilcox.test(combined_gaster_angle$mean_gaster_angle~combined_gaster_angle$Treatment)

#boxplot of choices
boxplot(combined_gaster_angle$vertical_mean_angle~combined_gaster_angle$Treatment,xlab="Treatment", ylab="Mean gaster angle (degrees)",main="Mean gaster angles for perturbed vs. unperturbed experiments",range=0)
...
################################################################
ANALYZING 1.5M AND 3M EXPERIMENTS YEAR WISE
################################################################

1.5M DATA ANALYSIS

FALL 2024
Sample size: n=10 (Sample size too low to do any meaningful analysis on this data alone)

FALL 2022
Sample size: n=40 (n=13 for control and n=27 for experimental)

Result: 
Significant results: Chi-square test for abandonment and binomial test for control for abandonment.

```{r}
#Choices
fall_2022_choices<-data.frame(fall_2022_1.5m_choices$`Treatment (Control/Experimental)`,fall_2022_1.5m_choices$`Ant choice`)

colnames(fall_2022_choices)<-c("treatment","choice")

#Converting this data into a table for the fisher's test
fall_2022_choices<-table(fall_2022_choices)

fisher.test(fall_2022_choices)
chisq.test(fall_2022_choices,correct = T)#Applying the correction since the sample size is low for each of the treatment combination
#Non-significant
...
#Abandonment
fall_2022_abandonment<-data.frame(fall_2022_1.5m$`Treatment (Control/Experimental)`,fall_2022_1.5m$abandonment)

colnames(fall_2022_abandonment)<-c("treatment","abandonment")

#Converting this into a table for fisher's test
fall_2022_abandonment<-table(fall_2022_abandonment)

fisher.test(setup1_abandonment)
chisq.test(fall_2022_abandonment,correct = F)
#Significant!

#Control 
binom.test(1,13,0.5)
#Significant: More ants stay on the trail than abandon

#Experimental
binom.test(18,27,0.5)
#Not significant: Ants here do not significantly abandon the trail more than staying on the trail. 

```

FALL 2023
Sample size: n=33 (n=15 for control and n=18 for experimental)

```{r}
#Choices
fall_2023_choices<-data.frame(fall_2023_1.5m_choices$`Treatment (Control/Experimental)`,fall_2023_1.5m_choices$`Ant choice`)

colnames(fall_2023_choices)<-c("treatment","choice")

#Converting this data into a table for the fisher's test
fall_2023_choices<-table(fall_2023_choices)

fisher.test(fall_2023_choices)
chisq.test(fall_2023_choices,correct = T)#Applying the correction since the sample size is low for each of the treatment combination
#Non-significant

#Binomial test for control data
binom.test(11,12,0.5)
#Significant!
...
#Abandonment
fall_2023_abandonment<-data.frame(fall_2023_1.5m$`Treatment (Control/Experimental)`,fall_2023_1.5m$abandonment)

colnames(fall_2023_abandonment)<-c("treatment","abandonment")

#Converting this into a table for fisher's test
fall_2023_abandonment<-table(fall_2023_abandonment)

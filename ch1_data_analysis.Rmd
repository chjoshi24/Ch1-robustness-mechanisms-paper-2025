...
###################################################################
ANALYZING CHOICES FOR 1.5M AND 3M EXPERIMENTS
###########################################################
...
#################################################################
ANALYZING ABANDONMENT DATA FOR 1.5M AND 3M EXPERIMENTS
##################################################################
...
################################################################
ANALYZING 1.5M EXPERIMENTS AS PER THE SETUP USED
Fall 2022 data was collected on a setup with a slightly smaller T-maze stem (with same T-maze arm length).
################################################################
Analyzing data from Fall 2022 alone

Sample size: n=40 (n=13 for control and n=27 for experimental)

Result:None of the tests are significant. This indicates that the overall sample size of Fall 2022 data was too low to infer a meaningful result.
...
setup1_abandonment<-data.frame(fall_2022_1.5m$`Treatment (Control/Experimental)`,fall_2022_1.5m$abandonment)

colnames(setup1_abandonment)<-c("treatment","abandonment")

#Converting this into a table for fisher's test
setup1_abandonment<-table(setup1_abandonment)
...
Analyzing Fall 2023 and Fall 2024 data together
Combined sample size:n=43 (n=19 for the control, and n=24 for experimental)

Result: 
(1) Choices result: Fisher's test was not significant indicating that there is no relationship between the treatments and the choices made by ants. But the Binomial test for the control trials alone were significant.
(2) Abandonment result: Fisher's test was significant. The binomial tests for both control and treatments were significant. 

Together this indicates that ants reliably used pheromone trails to reach the food source in the absence of perturbations. Perturbations resulted in more abandonment than their absence.
...
setup2_abandonment<-rbind(fall_2023_1.5m,fall_2024_1.5m)

setup2_abandonment<-data.frame(setup2_abandonment$`Treatment (Control/Experimental)`,setup2_abandonment$abandonment)

colnames(setup2_abandonment)<-c("treatment","abandonment")

#Converting this into a table for fisher's test
setup2_abandonment<-table(setup2_abandonment)

fisher.test(setup2_abandonment)
#Significant!
...
#Experimental
binom.test(21,24,0.5)
#Significant: More ants abandon the trail here than staying on the trail

```


RESULT FROM SEPERATELY ANALYZING 2022 DATA AND 2023+2024 DATA:
- Common significant result: Ants reliably use pheromone trail in the absence of the perturbation, indicating that the T-maze stem length does not affect ant's motivation to forage in absence of the perturbation.
- Abandonment is slightly higher in 2023+2024 data (87.5%) compared to 2022 data alone (67.5%).IMP point to note: The sample size for the experimental treatment for 2022 was slightly higher than 2023+2024 combined (n=27 for 2022, and n=24 for 2023+2024 combined). So maybe this could be due to seasonal differences in the behavior or longer T-maze length initiates more abandonment only when the pheromone trail is perturbed.

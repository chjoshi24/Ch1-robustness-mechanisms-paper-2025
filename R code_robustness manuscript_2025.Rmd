---
title: "R code_robustness manuscript"
author: "Chinmay Joshi and Anna Dornhaus"
date: "2025-12-23"
...
boxplot_low_sucrose <- ggplot(gaster_angle_low_sucrose_ready_1,
  aes(x = Treatment, y = mean_gaster_angle, fill = Ant.choice)) +
    # Boxplots with increased spacing
    geom_boxplot(position = position_dodge(width = 1.2),
                 width = 0.6, color = "black",
                 outlier.shape = NA, coef = Inf) +
    # Jittered points that align perfectly with the boxplots
    geom_jitter(
      aes(color = Year, group = Ant.choice),
    ...
    # Sample size labels
    geom_text(data = low_sucrose_sample_size_1,
              aes(x = Treatment, y = 60,
                  label = paste0("n=", n),
                  group = Ant.choice),
              position = position_dodge(width = 1.0),
              inherit.aes = FALSE) +
    # Significance symbol for Control–Correct
    geom_text(data = control_correct_symbol_low_sucrose,
              aes(x = Treatment, y = y,
                  label = label,
                  group = Ant.choice),
              position = position_dodge2(width = 1.0,
                                         preserve = "single"),inherit.aes = FALSE,
  ...
  theme(panel.background = element_blank(),
          ...
          axis.line        = element_line(color = "black")) +labs(title = "Low sucrose experiment",
         x = "Treatment",
         y = "Mean gaster angle",
         fill = "Ant choice")
...
aes(x = Treatment, y = mean_gaster_angle, fill = Ant.choice)) + 
  ...
  geom_boxplot(position = position_dodge(width = 1.0),
               width = 0.6, color = "black",
               outlier.shape = NA, coef = Inf) + 
  # Jittered points aligned perfectly with the boxplots
  geom_jitter(
    aes(color = Year, group = Ant.choice),
    position = position_jitterdodge(
      jitter.width = 0.40,
...
y = "Mean gaster angle",fill = "Ant choice") +
  # Sample size labels
  geom_text(data = high_sucrose_sample_size_1,
            aes(x = Treatment, y = 60,
                label = paste0("n=", n),
                group = Ant.choice),
            position = position_dodge(width = 1.0),
  ...
  geom_text(data = dunn_test_high_sucrose_letters_1,
            aes(x = Treatment, y = max(gaster_angle_high_sucrose_ready_1$mean_gaster_angle) + 5,
label = Letter,
                group = Ant.choice),
            position = position_dodge(width = 1.0),
            inherit.aes = FALSE) +
